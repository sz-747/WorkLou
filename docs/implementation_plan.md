...
### service_change_log — append-only service-data change history
| column | type | notes |
|---|---|---|
| id | uuid pk | |
...
The removed Admin interface previously wrote corrections to the shared `services` / `service_attributes` rows and appended log rows. Existing `admin_corrected` values remain valid historical data and are still ranked with `provider_confirmed` in matching. New background review decisions also append through `src/lib/service-history.ts`.
...
6. **Updater (Phase 7A, built 2026-09-05):** `runUpdater()` iterates active services; adapters normalise each machine-accessible source into a structured snapshot, compare it with canonical facts, and refresh unchanged values. Every value change or new fact becomes an `update_candidates` row; nothing is auto-applied. The protected `POST /api/updater/run?trigger=manual|scheduled` route, local compose sidecar, and prepared `runWorkLouUpdater` Base44 function can start runs. The Admin review screen has been removed, so queued candidates cannot currently be approved or rejected in the app. Existing backend decision functions remain for a future reviewed interface. Machine HTTP triggers require `SCHEDULER_SECRET`. Adapter failures are logged and never corrupt canonical data.
7. **Discovery (Phase 7B, built 2026-09-05):** `runDiscovery()` uses the protected `POST /api/discovery/run?trigger=manual|scheduled` route to search for provider pages, extract structured candidates, deduplicate them, and queue them with provenance. It never changes canonical service data by itself. The Admin review screen has been removed, so queued candidates cannot currently be approved or rejected in the app. Existing backend decision functions remain for a future reviewed interface. The local compose sidecar and prepared `runWorkLouDiscovery` Base44 function can start bounded scheduled runs. Machine HTTP triggers require `SCHEDULER_SECRET`; repeated runs do not duplicate candidates.
8. **Spreadsheet migration (Phase 8, built 2026-09-05):** `src/lib/spreadsheet.ts` can parse and stage a CSV verbatim, then import or discard each row without overwriting existing canonical values. The removed Admin screen was the only upload and review interface, so this flow is not currently reachable from the app. `GET /api/services/export` still exports the canonical directory as CSV.
...
- **Historical Admin correction propagation was verified live before the interface was removed.** Existing corrected data remains readable by caseworker matching.

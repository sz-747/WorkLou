...
- Bright Data adapters (`src/lib/brightdata.ts`, SERP API + Web Unlocker over `api.brightdata.com/request`): need `BRIGHT_DATA_API_KEY`, `BRIGHT_DATA_SERP_ZONE`, `BRIGHT_DATA_UNLOCKER_ZONE` env (optional; fixtures/direct sources work without them). There is NO `BRIGHT_DATA_DATASET_ID` dependency. Adapter tests run offline with stubbed HTTP (`npm run db:test:brightdata`).
